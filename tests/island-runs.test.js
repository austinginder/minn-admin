/**
 * In-place island text editing (nested-content plan, phase 2): a complex
 * island's preview arms editable text runs; edits splice into the stored
 * raw and save byte-faithfully. Guards: Enter, markdown wraps, slash menu,
 * toolbar marks and rich paste are all blocked inside runs (text-only by
 * design); ⌘A clamps to the run. Fixtures are FOREIGN wrapper blocks —
 * since the nesting endgame, group/columns always slot (their prose is
 * directly editable), so runs belong to non-container islands now.
 */
...
( async () => {
	...
	const content = [
		'<!-- wp:acme/panel -->',
		'<div class="acme-panel"><h2 class="wp-block-heading">Welcome, Michala!</h2><p>Reminders are in the Noted link.</p><ul class="wp-block-list"><li>First reminder item</li></ul><div class="acme-badge">Badge text</div></div>',
		'<!-- /wp:acme/panel -->',
		'',
		'<!-- wp:acme/duo -->',
		'<div class="acme-duo"><div class="acme-col"><p>Left column text.</p></div><div class="acme-col"><p>Right column text stays untouched.</p><div class="acme-badge">Column badge</div></div></div>',
		'<!-- /wp:acme/duo -->',
		'',
		'<!-- wp:paragraph -->',
	...
	await page.waitForSelector( '#minn-editor-body .minn-block-island[data-block="acme/panel"] .minn-island-run', { timeout: 25000 } );

	const shape = await page.evaluate( () => {
		const g = document.querySelector( '.minn-block-island[data-block="acme/panel"]' );
		const c = document.querySelector( '.minn-block-island[data-block="acme/duo"]' );
		return {
	...
	t.check( 'panel island arms four runs', shape.groupRuns === 4, JSON.stringify( shape ) );
	t.check( 'duo island arms three runs', shape.colRuns === 3, JSON.stringify( shape ) );
	...
	// 1. Heading edit.
	await page.click( '.minn-block-island[data-block="acme/panel"] h2 .minn-island-run' );
	await caretEnd( '.minn-block-island[data-block="acme/panel"] h2 .minn-island-run' );
	await page.keyboard.type( ' and friends' );

	// 2. Markdown must stay literal inside a run (bindMarkdown is blocked).
	await page.click( '.minn-block-island[data-block="acme/panel"] p .minn-island-run' );
	await caretEnd( '.minn-block-island[data-block="acme/panel"] p .minn-island-run' );
	await page.keyboard.type( ' Stay **calm** here.' );
	const pHtml = await page.$eval( '.minn-block-island[data-block="acme/panel"] p .minn-island-run', ( el ) => el.innerHTML );
	...
	await page.click( '.minn-block-island[data-block="acme/panel"] li .minn-island-run' );
	await caretEnd( '.minn-block-island[data-block="acme/panel"] li .minn-island-run' );
	await page.keyboard.press( 'Enter' );
	const liHtml = await page.$eval( '.minn-block-island[data-block="acme/panel"] li .minn-island-run', ( el ) => el.innerHTML );
	...
	const selShape = await page.evaluate( () => {
		...
		const run = document.querySelector( '.minn-block-island[data-block="acme/panel"] li .minn-island-run' );
	...
	const liAfterBold = await page.$eval( '.minn-block-island[data-block="acme/panel"] li .minn-island-run', ( el ) => el.innerHTML );
	t.check( 'toolbar marks refused in run', ! /<(strong|b)>/i.test( liAfterBold ), liAfterBold );

	// 7. Rich paste lands as plain text.
	await caretEnd( '.minn-block-island[data-block="acme/panel"] li .minn-island-run' );
	await page.evaluate( () => {
		const el = document.querySelector( '.minn-block-island[data-block="acme/panel"] li .minn-island-run' );
		const dt = new DataTransfer();
		dt.setData( 'text/html', '<b>RICH</b> paste' );
		dt.setData( 'text/plain', 'RICH paste' );
		el.dispatchEvent( new ClipboardEvent( 'paste', { clipboardData: dt, bubbles: true, cancelable: true } ) );
	} );
	const liAfterPaste = await page.$eval( '.minn-block-island[data-block="acme/panel"] li .minn-island-run', ( el ) => el.innerHTML );
	...
	await page.click( '.minn-block-island[data-block="acme/duo"] .minn-island-run' );
	await caretEnd( '.minn-block-island[data-block="acme/duo"] .minn-island-run' );
	...
	for ( let i = 0; i < 8; i++ ) {
		...
		colText = await page.$eval( '.minn-block-island[data-block="acme/duo"] .minn-island-run', ( el ) => el.textContent );
	...
	t.check( 'panel comment pair intact', raw.includes( '<!-- wp:acme/panel -->' ) && raw.includes( '<!-- /wp:acme/panel -->' ), '' );
	t.check( 'duo comment pair + wrappers intact', raw.includes( '<!-- wp:acme/duo -->' ) && raw.includes( '<div class="acme-duo"><div class="acme-col">' ), '' );
	...
	const runCopy = await page.evaluate( () => {
		const run = document.querySelector( '.minn-block-island[data-block="acme/duo"] .minn-island-run' );
		const r = document.createRange();
