...
( async () => {
	...
	try {
		...
		t.check( 'a single-status list does not chip the dropdown status',
			! ( await chipLabels() ).some( ( c ) => /^Status:/i.test( c ) ), JSON.stringify( await chipLabels() ) );
		...
		t.check( 'featured filter keeps only featured products',
			await waitRows( [ made.featured, made.low ], [ made.sale, made.variable ] ), JSON.stringify( await visibleIds() ) );
		...
		await pickChoice( 'featured', 'true' );
		t.check( 'low stock plus featured still lists the low featured product',
