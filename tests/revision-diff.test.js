...
( async () => {
	...
	/* ===== Fixture. REST CREATE stores no revision — only updates snapshot,
	   and each revision holds the post state AFTER that update. So:
	   create placeholder → update to V1 (revision:V1) → update to V2
	   (revision:V2, post:V2). Oldest revision is the V1 we diff against. ===== */
	...
	t.check( 'history card lists revisions', revRows.length >= 2, String( revRows.length ) );

	/* ===== Oldest revision (v1) vs current (v2) ===== */
	...
	/* ===== Newest revision now matches current → identical notice ===== */
	await openEditor( page, id );
	await page.waitForSelector( '.minn-history-row', { timeout: 15000 } );
	await ( await page.$( '.minn-history-row' ) ).click();
	await page.waitForSelector( '#minn-modal-overlay .minn-side-row', { timeout: 10000 } );
	await page.waitForTimeout( 400 );
	const idText = await page.evaluate( () => [ ...document.querySelectorAll( '#minn-modal-overlay .minn-side-row' ) ].map( ( r ) => r.textContent ).join( ' ' ) );
	t.check( 'identical revision says so', /Identical to the current content/.test( idText ), idText.slice( 0, 140 ) );
