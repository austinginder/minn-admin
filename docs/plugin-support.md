...
| **Bookings** | Amelia | **Bookings** surface — upcoming appointments as an inbox (pending / today / canceled filters, search, approve / cancel / no-show through Amelia's own status handler so notifications stay its own), a contact card with service, employee and time, a status card of today / pending / next, and an Open Amelia deep link. Calendar authoring, employee hours and the booking form stay in Amelia |
...
1. **LMS** — LearnDash (paid, StellarWP), Tutor LMS (~100k), LifterLMS.
   ...
   thesis.
2. **Bookings** — **Amelia SHIPPED** (appointments inbox + status actions +
   Open Amelia). Bookly, FluentBooking, Simply Schedule Appointments remain
   family joiners. Appointments are the most inbox-shaped data there is.
