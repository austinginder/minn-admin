...
| **Spam filtering** | Akismet, Antispam Bee, CleanTalk, WP Armour | Settings → Spam provider cards; open via `minn_admin_spam_providers`. **Akismet** and **CleanTalk** paste their cloud keys on the card (and as key-shaped rows on Licenses). **CleanTalk** also marks existing spam accounts: Users gains a Spam tab of those results, with Not spam / delete on the row (posts go with the account, the way CleanTalk does it) and a Check ↗ to their scan |
