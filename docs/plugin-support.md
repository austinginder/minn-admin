...
1. **Backups providers** — WPvivid shipped (list/status/schedule/backup-now/
...
2. **Email log providers** — GoSMTP (logs free), SureMails, Site Mailer;
   Easy WP SMTP's full log is Pro-only (free has debug events, the
   WP Mail SMTP shape).
3. **Snippets providers** — Simple Custom CSS & JS (a CPT) and Header Footer
   Code Manager (`hfcm_scripts` table) into the existing Snippets surface.
4. **Security leftover** — All-In-One Security (activity-log family +
   posture row; the LLA-R / Solid Security pattern).
5. **Forms leftovers** — SureForms and MetForm (free-tier entry storage
   believed but not source-verified; verify before promising).
6. **Bigger scoped bets** — WPForms Pro entries (source-verified: Lite
