...
| **Bookings** | Amelia, LatePoint, Bookly | **Bookings** surface — upcoming appointments as an inbox (pending / today / canceled filters, search, approve / cancel / no-show through each plugin's own status handler so notifications stay its own; Bookly has no no-show status, so it offers approve / cancel), a contact card with service, employee and time, a status card of today / pending / next, and an Open Amelia / Open LatePoint / Open Bookly deep link. Calendar authoring, employee hours and the booking form stay in the plugin |
