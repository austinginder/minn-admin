...
| **Gift cards** | YITH WooCommerce Gift Cards (free and premium) | **Gift cards** surface — YITH has no REST at all (its `gift_card` CPT is `show_in_rest => false` and it registers no routes), so Minn's own shim reads the posts and writes through YITH's own object. The list wears the **orders filter bar** (`filterBar`), not a pill strip: a status dropdown that holds more than one status at once, Add filter with a date window, chips and the whole narrowing in the URL, plus a search that reaches both the code and the recipient, a status card leading with the **outstanding balance** the store still owes, a detail card (code, balance, original amount, expiry, recipient, message, the order it came from), three verbs through YITH's own methods (enable/disable, adjust balance, resend the gift card email), and an **Add gift card** modal that issues one by hand: amount, an optional code YITH generates when left blank, recipient, message, and the option to email it there and then. The gift-card product's own amount options, bulk generation, CSV, PDF and QR stay in YITH, one deep link away. See [yith-gift-cards.md](yith-gift-cards.md) |
