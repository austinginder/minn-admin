...
| **Search suggestions** | JetSearch | **Search suggestions** surface in Tools: what visitors search ordered by weight, add / edit / delete, and the plugin's own merge-duplicates sweep (weights added onto the first entry) from the status card |
| **Filters** | JetSmartFilters | **Filters** surface in Tools: the filters list with the type name from their registry and the query variable, Edit ↗ to their editor, trash, and the indexer on the status card (on / off, rows indexed, Reindex through their own index_filters) |
