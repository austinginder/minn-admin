...
| CleanTalk Anti-Spam | activate / deactivate / verify | bogus key (live cleantalk.org refusal, nothing written; also driven through the spam-card paste field) | 2026-08-20 | CLOUD ACCESS KEY, same shape as Akismet; `ct_account_status_check` then `apbct_settings__save_key($key, true)`; settings.php require'd under REST; snapshot-restore of `cleantalk_data`/`cleantalk_settings` so a rejected paste cannot clobber a working key (their check writes `key_is_ok` even for a candidate); `CLEANTALK_ACCESS_KEY` constant is read-only |
