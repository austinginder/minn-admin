...
| Novamira Pro | WooCommerce Software License client (`woo_sl_action` requests to license.dynamic.ooo, product `WP-NVP-1`): plain options `nvp_license_key` (lowercased), `nvp_license_status` ('active' or ''), `nvp_license_error` (the server's last message, cleared on success), `nvp_license_domain` (where it validated). Their cron re-checks twice daily and auto-repairs an expired-but-renewed key | Valid when status is active (note when the stored domain is not this site); the error text classifies expired vs invalid; key with no status is unknown |
...
- **SEOPress Pro** (`seopress-pro`): stock EDD against seopress.org
  (`STORE_URL_SEOPRESS` / `ITEM_ID_SEOPRESS` 113). Key in the wp-config
  `SEOPRESS_LICENSE_KEY` constant (wins) or the `seopress_pro_license_key`
  option; `seopress_pro_license_status` + `seopress_pro_license_expiry`
  (epoch; lifetime stored as +100 years) beside it. The generic sweep's
  token (`wp_seopress`) can never match the `seopress_pro_` option prefix,
  so a parked Pro plugin's stored license was invisible. Actions mirror
  their own WP-CLI License command request-for-request (`url` is the `home`
  option, not site_url; deactivate treats the server's 'failed' as
  already-inactive success and then runs their full reset including the
  EDD-cache flush; a constant-defined key refuses paste and survives
  deactivation). Verify is read-only (the Automatic.css dev-clone lesson).
...
| **Crocoblock** | ~~JetEngine + the Jet* pack~~ (SHIPPED 2026-09-02, real-key full loop; see the tested table) | One license for ~20 Elementor-ecosystem plugins; one provider row through the shared Jet Dashboard framework. |
...
| SEOPress Pro | activate / deactivate / verify | bogus key + read against a real stored license (etch lab) | 2026-08-21 | mirrors their own WP-CLI License command; real-key pass is the owner's step (the lab's Pro is parked at 9.7.3 under free 10.1) |
...
| CleanTalk Anti-Spam | activate / deactivate / verify | bogus key (live cleantalk.org refusal, nothing written; also driven through the spam-card paste field and Settings → Connectors) | 2026-08-20 | CLOUD ACCESS KEY, same shape as Akismet; registered as a `spam_filtering` connector next to core's Akismet; `ct_account_status_check` then `apbct_settings__save_key($key, true)`; settings.php require'd under REST; snapshot-restore of `cleantalk_data`/`cleantalk_settings` so a rejected paste cannot clobber a working key (their check writes `key_is_ok` even for a candidate); `CLEANTALK_ACCESS_KEY` constant is read-only |
| WooCommerce.com, Site Kit, Jetpack, SureCart connections | read + Connect ↗ link-out | read (live states on minnadmin: WC unconnected → missing, Site Kit half-connected → unknown, Jetpack off) | 2026-08-06 | connections-center brick 3; presence-only reads, tokens never decoded; no actions by design (OAuth stays on vendor screens). SureCart added 2026-08-12: its store token (`sc_api_token`) is read for PRESENCE only, never decrypted, and everything SureCart sells lives in their cloud behind it |
...
| WP Migrate | activate / deactivate / verify | real key, full loop against api.deliciousbrains.com (bad key refused with nothing stored; real key stored with their own expired semantics; verify refreshed; deactivate cleared) | 2026-08-19 | Delicious Brains / WP Engine. THREE key stores read in their precedence: `WPMDB_LICENCE` constant, then USER META `wpmdb_licence_key`, then `wpmdb_settings['licence']` — their own activation writes user meta and unsets the global option, so the key is per person. Validity is the cached API answer in the per-user site transient `wpmdb_licence_response_{uid}`; their API returns NO expiry date in any response, so rows state condition without a date. Their activate deliberately STORES the key on subscription_expired / activation_deactivated / api-down; mirrored rather than simplified. No deactivate endpoint exists in their API at all, so removal is local and the message says the seat is not freed. License.php and Helpers.php are byte-identical 2.7.4 → 2.7.11 |
| Independent Analytics Pro | activate / deactivate / verify | real key, full loop against api.freemius.com (bad key refused with nothing stored; real key → Valid, lifetime, activation count; verify; deactivate released the license and the plan reverted to free; re-activated) | 2026-09-01 | FIRST FREEMIUS VENDOR WITH ACTIONS, through the SDK the plugin ships (`IAWP_FS()`). Read is pure `fs_accounts` (site entry → `all_licenses[9944]`; the entity stores the key as `secret_key`, only presence leaves the reader). Activate: registered install → `activate_migrated_license()`; NEVER-CONNECTED install → `opt_in( …, $key, …, $redirect = false )`, because the migrated path threads `$redirect = true` into `setup_account()` → `fs_redirect()` → exit mid-request. Deactivate and verify reach the protected `_deactivate_license()` / private `_sync_license()` by reflection (their only callers are nonce-checked GET handlers); documented exception, Soflyy precedent. The three `minn_admin_freemius_*` helpers are generic: any other Freemius product is a reader plus three one-line closures around its `*_fs()` accessor. Localhost activations are free in Freemius (`activated_local`), so a licensed dev site reads "0 of N activations used" |
| Novamira Pro | activate / deactivate / verify | bogus key only (no key in hand): `activate_new_license_key()` reached license.dynamic.ooo and answered "The License Key is invalid" cleanly; the previous key/status/error/domain were restored (their flow overwrites the key BEFORE checking, so a typo would otherwise replace a working key). Real-key pass pending | 2026-09-16 | Their functions return `[ ok, message ]` after a live status check. Deactivate is `deactivate_license()` (frees the seat; on their staging-domain answer it just clears the local key). Verify is `refresh_license_status()` then `is_license_active()`. Messages map: expired → expired, domain/another site → site_limit, else invalid |
| Crocoblock (JetEngine + the whole Jet pack) | activate / deactivate / verify | real key, full loop against api.crocoblock.com (bad key refused with nothing stored; real key → Valid, lifetime, covers all 20 installed Jet plugins; verify resynced; deactivate removed the site from the license; re-activated) | 2026-09-02 | ONE membership key for every Jet plugin, so one row carries the family. Storage is option `jet-license-data['license-list']`, keyed by the license key: `{licenseStatus, licenseKey, licenseDetails {type crocoblock/tm/envato, product_category, expire ('0000-00-00 00:00:00' = never), site_url, plugins {file => …}, activation_limit, is_sublicense}}`; the read is pure options and takes the key's presence only. Actions ride the shared `Jet_Dashboard` framework every Jet plugin bundles (newest copy boots): it is `is_admin()`-gated, so under REST `Dashboard::get_instance()` loads its inc/ files (init already passed, no managers boot) and a constructor-less `License_Manager` (reflection; its constructor fires notices and a network sync through managers that never booted) exposes the public `license_action_query()` + `update_license_list()` their ajax handler uses. A SUBLICENSE key is answered with the PARENT key and stored under that, so deactivate/verify use the stored key, never the pasted one. `site_url` mismatch (clone or migration) reads as invalid with the reason, which is what their own "site is not activated" notice means. The activation response lists every other site on the license; their handler strips it before storing and Minn never surfaces it |
