...
| `group` | Sidebar placement. Four values are honored: the default **`"tools"`** (logs, redirects, snippets: site plumbing), **`"workspace"`** for inbox-shaped publishing work such as form entries, **`"commerce"`** for orders, appointments and other store operations, and **`"network"`** for multisite surfaces that belong to the whole network rather than one site. Since v0.17.0 workspace is enforced, not asked: it requires a collection with an `ago` column (a time-ordered stream); anything else is placed under Tools and the Integrations card says so. The same budget applies at the other end: a surface that is settings-ONLY (a `settings` block and no `collection`) and names no `family` does not get a sidebar item at all. It folds into the single **Site Options** item, contributing its tabs, so configuration cannot bleed into navigation one plugin at a time. Your route and capability are untouched (each folded tab carries its own `route`, resolved and given a collision-proof id), and declaring a `family` opts you out when your settings genuinely belong to a topic that has other members (Perfmatters sits with Performance Lab under Performance rather than under options). A `network` surface is only as protected as its `cap`, so gate it on a network capability (`manage_sites`, `manage_network_options`) or it will show up for ordinary site administrators |
...
### `dataForm` — form-edit fields nested inside one object attribute

Some blocks keep their real content nested inside a single object attribute rather than
as top-level attributes. ACF blocks are the canonical case: the comment carries
`"data": { "field": value, "_field": "field_key" }` and the generic form rightly skips
object attributes as too structural. A `dataForm` descriptor names that attribute and
describes its subfields, and the inspector renders them as a real form:

```php
$forms['acf/testimonial'] = array(
    'attributes' => array(
        'name' => array( 'hide' => true ),  // wrapper plumbing — hide it
        'data' => array( 'hide' => true ),
        'mode' => array( 'hide' => true ),
    ),
    'dataForm'   => array(
        'attr'   => 'data',                          // the object attribute
        'fields' => array(
            array( 'name' => 'author', 'label' => 'Author', 'control' => 'text' ),
            array( 'name' => 'tone', 'label' => 'Tone', 'control' => 'select',
                   'options' => array( array( 'light', 'Light' ), array( 'dark', 'Dark' ) ) ),
        ),
        'alias'  => array( 'author' => 'field_abc123', 'tone' => 'field_def456' ),
        'locked' => 2,                               // fields the form can't render
    ),
);
```
