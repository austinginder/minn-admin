...
v0.13.0 cycle (WP 7.0.1); re-checked 2026-07-13 at v0.14.0 open: every
ranked priority remains shipped, no new daily-work gaps in core areas.
Dev tools expansion (Scrutoscope, cron/transients inventory) is adapter
work, not a core gap — see `plugin-support.md` Wave A. Minn's positioning
grades these: daily work belongs in Minn, the long tail stays one click
away in wp-admin. Each area below gets a status and a judgment on whether
the gap blocks daily work.
