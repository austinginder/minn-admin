...
| 1 | **ecommerce suite re-run vs WC 11.0** | ops | Major-version smoke beyond list routes (orders detail, refunds, coupons, subscriptions views) | S | wc/v3 lists are 200 but a major deserves the full suite before the next release |
| 2 | forms status cards (fluent / ninja / forminator / flamingo) | A | Status-card parity with SureForms | S each | Carried from 07-27; optional polish, only if dogfooding wants inbox counts |
| 3 | WPForms Pro entries | B | Forms family | M–L | Still license-blocked; biggest uncovered forms name |
