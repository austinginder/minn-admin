...
| jetpack-stats | Traffic provider (+ day drill) | Y | 2026-07-29 | WPCOM_Stats blog-token client (their 5-min cache); gates: connected + stats module + `view_stats`; priority 20 (Site Kit tier); per-page counts views-only by WPCOM design; verified against the mu-fixture WPCOM mock — real connected-site pass still worth one manual look; installed-inactive |
