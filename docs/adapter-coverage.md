...
|---|---|---|---|---|---|
| 1 | **fluent-smtp** | B | Resend through their `Logger::resendEmailFromLog()` + resend-history rows in the sections detail (`resent_count` + `extra['resends']` trail; read via their model, never raw unserialize). Optional: recipient-override resend via an action `fields` form, matching their 2.3.0 Recipient Picker | S–M | 2.3.0 built the exact API our `wp_mail` resend bypasses; today Minn resends leave no trail in their log UI |
| 2 | **fluent-smtp** | B | Connection-health rows on the status card via `ConnectionHealth::getReport()`/`getFailing()`; consider a System health row (backups precedent) when a connection is failing | S | Their daily cron already computes it; option-backed read, zero network |
| 3 | **fluent-smtp** | B | Honor `fluent_mail/manage_capability` (three hardcoded `manage_options` sites in the adapter) | S | Checklist #9: vendor cap model now exists; trivial diff |
| 4 | **ecommerce suite re-run vs WC 11.0** | ops | Major-version smoke beyond list routes (orders detail, refunds, coupons, subscriptions views) | S | wc/v3 lists are 200 but a major deserves the full suite before the next release |
| 5 | forms status cards (fluent / ninja / forminator / flamingo) | A | Status-card parity with SureForms | S each | Carried from 07-27; optional polish, only if dogfooding wants inbox counts |
| 6 | WPForms Pro entries | B | Forms family | M–L | Still license-blocked; biggest uncovered forms name |
| — | elementor-pro + woocommerce-subscriptions updates | ops | License-gated (4.1.2 vs 4.2.1; 9.0.1 vs 9.1.0) | — | Needs Austin's licenses; free Elementor now 4.2.1 so free/Pro skew persists |

Ranks 1–3 are one adapter and compose naturally into a single "FluentSMTP 2.3.0
catch-up" ship (`sweep --ship top 1` = all three slices + suite + docs).
